    ...
    <article className="group relative rounded-lg overflow-hidden bg-surface-secondary mb-4">
      ...
      >
        ...
        >
          ...
          <div className="absolute inset-0 bg-gradient-to-b from-black/0 via-black/0 to-black/50 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
        </div>
      </Link>

      {/* Actions (visible on hover for desktop, always visible on mobile) */}
      <div className="absolute top-3 right-3 flex items-center gap-1.5 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity duration-200">
        <SaveButton photo={photo} size="sm" />
      </div>

      {/* Bottom bar */}
      <div className="absolute bottom-0 left-0 right-0 p-3 flex items-center justify-between opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity duration-200">
        {/* Photographer */}
        <Link
